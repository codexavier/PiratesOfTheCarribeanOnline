...
class Pistol(Weapon.Weapon):
    ...
    def getAmmoModel():
        ...
        bullet.setScale(0.29999999999999999)
        return bullet
    ...
    def getReturnIval(self, av, blendInT = 0, blendOutT = 0.10000000000000001):
        track = Parallel(Func(base.playSfx, self.returnSfx, node = av, cutoff = 60), av.actorInterval('gun_putaway', playRate = 2, endFrame = 37, blendInT = blendInT, blendOutT = blendOutT), Sequence(Wait(0.59999999999999998), Func(self.detachFrom, av)))
    ...
    def startPulseGlow(self):
        if not self.pulseGlow:
            ...
            if self.pulseGlow:
                ...
                self.pulseGlow.setEffectColor(Vec4(1, 0.90000000000000002, 0.80000000000000004, 0.5))
                self.pulseGlow.setPos(0.20000000000000001, 0.25, 0)
                self.pulseGlow.setScale(0.29999999999999999)
