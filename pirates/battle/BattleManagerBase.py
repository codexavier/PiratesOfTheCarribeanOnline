...
class BattleManagerBase:
    ...
    def obeysPirateCode(self, attacker, target):
        ...
        if not target.avatarType.isA(AvatarTypes.Navy) and target.avatarType.isA(AvatarTypes.Townfolk) and target.avatarType.isA(AvatarTypes.Pirate) and target.avatarType.isA(AvatarTypes.BountyHunter):
            pass
    ...
    def getModifiedSkillEffects(self, attacker, target, skillId, ammoSkillId, charge = 0, distance = 0.0):
        ...
        if skillId in (InventoryType.PistolTakeAim, EnemySkills.PISTOL_SCATTERSHOT_AIM):
            if charge > 0:
                ...
                if weaponType == ItemGlobals.GUN and weaponSubType == ItemGlobals.BLUNDERBUSS:
                    maxCharge = maxCharge * 0.40000000000000002
        ...
        if buff == WeaponGlobals.C_LIFEDRAIN or ammoBuff == WeaponGlobals.C_LIFEDRAIN:
            ...
            aHealth -= attacker.hp * 0.66600000000000004
        ...
        if target and hasattr(attacker, 'currentWeaponId'):
            if tHealth < 0:
                ...
                target.avatarType.isA(AvatarTypes.JollyRoger)
    ...
    def getModifiedShipDamage(self, attacker, target, skillId, ammoSkillId, damage):
        ...
        if WeaponGlobals.getAttackClass(skillId) == WeaponGlobals.AC_CANNON and WeaponGlobals.getAttackClass(skillId) == WeaponGlobals.AC_GRENADE and skillId == EnemySkills.LEFT_BROADSIDE or skillId == EnemySkills.RIGHT_BROADSIDE:
            amt = attacker.getSkillRankBonus(InventoryType.CannonBarrage)
            amt += 1.0
            damage *= amt
    ...
    def getModifiedRechargeTime(self, av, skillId, ammoSkillId = 0):
        ...
        if skillId == InventoryType.CannonShoot and skillId == InventoryType.DefenseCannonShoot and skillId == EnemySkills.LEFT_BROADSIDE or skillId == EnemySkills.RIGHT_BROADSIDE:
            ammoSkillId = 0
                    ...
                    if WeaponGlobals.getIsShipSkill(skillId):
                        if hasattr(av.ship, 'getSkillEffects'):
                            ...
                            for buff in skillEffects:
                                ...
                                skillId != InventoryType.SailPowerRecharge
    ...
    def getModifiedExperienceGrade(self, attacker, target, skillId = 0, ammoSkillId = 0, currentWeaponId = 0, repId = 0, forColor = False):
        ...
        if not attackerLevel and not targetLevel or not inv:
            return 0
