...
class TattooStoreCartList(DirectScrolledFrame):
    ...
    def addPanel(self, data, repack = 1):
        ...
        panel = DirectButton(parent = self, relief = None, text = itemText[:maxLength], text_fg = self.itemColor, text_align = TextNode.ALeft, text_scale = PiratesGuiGlobals.TextScaleMed, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.059999999999999998, 0.0), command = self.removePanel, extraArgs = [
            data])
        panel.costLabel = DirectLabel(parent = panel, relief = None, text = str(itemCost), text_fg = self.itemColor, text_align = TextNode.ARight, text_scale = PiratesGuiGlobals.TextScaleMed, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.45000000000000001, 0.0), image = self.parent.CoinImage, image_scale = 0.14999999999999999, image_pos = (0.47999999999999998, 0.0, 0.014))
...
class TattooStoreGUI(DirectFrame):
    ...
    def __init__(self, npc, shopId, **kw):
        ...
        self.purchaseInventory = TattooStoreCartList(self, self.cartWidth, self.cartHeight - 0.94999999999999996, self.cartWidth, self.cartHeight / 20.0)
        self.purchaseInventory.reparentTo(self.cartFrame)
        self.purchaseInventory.setPos(0.0, 0.0, 0.76000000000000001)
        self.sellInventory = TattooStoreCartList(self, self.cartWidth, self.cartHeight - 0.94999999999999996, self.cartWidth, self.cartHeight / 20.0)
        ...
        self.nextPageButton = DirectButton(parent = self.panel, relief = None, state = DGG.DISABLED, image = triangle, image_scale = 0.065000000000000002, pos = (0.54000000000000004, 0.0, 0.17499999999999999), rolloverSound = None, command = self.nextPage)
        self.prevPageButton = DirectButton(parent = self.panel, relief = None, state = DGG.DISABLED, image = triangle, image_scale = -0.065000000000000002, pos = (0.16, 0.0, 0.17499999999999999), rolloverSound = None, command = self.previousPage)
    ...
    def focusCamera(self, cameraId = BODY_CAMERA):
        ...
        if cameraId == BODY_CAMERA:
            ...
            dummy.setPos(dummy, 0, 3, self.pirate.headNode.getZ(self.pirate) * 1.1000000000000001)
        else:
        ...
        if cameraId == BODY_CAMERA:
            ...
            dummy.lookAt(self.pirate, self.pirate.headNode.getX(self.pirate), self.pirate.headNode.getY(self.pirate), self.pirate.headNode.getZ(self.pirate) * 1.1000000000000001)
        else:
    ...
    def addTab(self, id):
        ...
        if id == ZONE1:
            ...
            tabIcon = self.TattooIcons.find('**/icon_shop_tailor_arm')
            tabScale = (-0.40000000000000002, 0.40000000000000002, 0.40000000000000002)
        elif id == ZONE4:
    ...
    def setPage(self, pageName, startIndex = 0, refreshWardrobe = True):
        self.tabBar.unstash()
        self.titleLabel['text'] = 'smallCaps' + self.rootTitle + ' - ' + TattooZones[pageName][1]
        ...
        for tattoo in tattoos:
            if self.tattooAmount - startIndex < self.buttonsPerPage and self.tattooAmount >= startIndex:
                ...
                if tex:
                    ...
                    cm.setFrame(-0.22, -0.040000000000000001, -0.071999999999999995, 0.078)
                ...
                tattooButton.itemText = DirectFrame(parent = tattooButton, relief = None, text = '', text_fg = PiratesGuiGlobals.TextFG1, text_align = TextNode.ACenter, text_scale = PiratesGuiGlobals.TextScaleSmall, text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = True, pos = (-0.13, 0, -0.085000000000000006))
                ...
                if questDrop is True and self.mode == SELLING:
                    ...
                    tattooButton.itemText['image_scale'] = 0.20000000000000001
                    tattooButton.itemText['image_pos'] = (-0.12, 0, 0.14999999999999999)
                ...
                if self.mode == BUYING and not owned:
                    if self.pvpMode or landInfamyRequired or seaInfamyRequired:
                        ...
                        tattooButton['text_pos'] = (-0.02, 0.050000000000000003)
                ...
                if not self.paid:
                    ...
                    tattooButton.addToCart['geom_scale'] = 0.20000000000000001
                    tattooButton.addToCart['geom_pos'] = Vec3(-0.10000000000000001, 0.0, 0.0)
