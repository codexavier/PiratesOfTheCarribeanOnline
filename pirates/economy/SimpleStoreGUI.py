...
class SimpleStoreColorPicker(BorderFrame):
    ...
    def setup(self):
        ...
        for idx in range(0, numcolors):
            color = colorSet[idx]
            colorButton = GuiButton.GuiButton(parent = self, pos = (offsetx, 0.0, offsety), image = (charGui.find('**/chargui_frame04'), charGui.find('**/chargui_frame04_down'), charGui.find('**/chargui_frame04_over')), image_scale = (0.115, 0.0, 0.087999999999999995), helpText = PLocalizer.TailorColorStrings.get(color), helpDelay = 0, command = self.selectColor)
            colorFrame = DirectFrame(parent = colorButton, frameSize = (-0.025000000000000001, 0.025000000000000001, -0.025000000000000001, 0.025000000000000001))
            colorButton['extraArgs'] = [
                color]
            colorFrame['frameColor'] = ItemConstants.DYE_COLORS[color]
            offsetx += 0.070000000000000007
...
class SimpleStoreBuyPanelGUI(BorderFrame):
    ...
    def setupGui(self):
        ...
        self.costLabel = DirectLabel(parent = self, relief = None, state = DGG.DISABLED, image = self.store.CoinImage, image_scale = 0.14999999999999999, image_pos = Vec3(0.38, 0, 0.012), text = PLocalizer.SimpleStoreCost, text_scale = PiratesGuiGlobals.TextScaleLarge, text_align = TextNode.ALeft, text_fg = PiratesGuiGlobals.TextFG2, text_shadow = PiratesGuiGlobals.TextShadow, text_wordwrap = 11, text_pos = (0, 0, 0.0), pos = (-0.20000000000000001, 0, -0.050000000000000003), text_font = PiratesGlobals.getInterfaceFont(), textMayChange = 1)
        self.costText = DirectLabel(parent = self, relief = None, state = DGG.DISABLED, text = str(costText), text_scale = PiratesGuiGlobals.TextScaleLarge, text_align = TextNode.ARight, text_fg = PiratesGuiGlobals.TextFG2, text_shadow = PiratesGuiGlobals.TextShadow, text_wordwrap = 11, text_pos = (0, 0, 0.0), pos = (0.14000000000000001, 0, -0.050000000000000003), text_font = PiratesGlobals.getInterfaceFont())
...
class SimpleStoreGUI(DirectFrame):
    ...
    guiPos = (0.625, 0.0, -0.050000000000000003)
    guiWidth = 1.25
    guiHeight = 1.3500000000000001
    ...
    def __init__(self, storeName, npc = None, shopId = None, **kw):
        ...
        self.backTabParent.setPos(-0.5 * self.guiWidth, 0.0, -0.40000000000000002 * self.guiHeight)
        textScale = PiratesGuiGlobals.TextScaleTitleSmall
        self.cartFrame = BorderFrame(parent = self, state = DGG.DISABLED, frameSize = (-(self.guiWidth) / 2, self.guiWidth / 2, -(self.guiHeight) / 2, self.guiHeight / 2), text = PLocalizer.InventorySellTitle, text_align = TextNode.ACenter, text_font = PiratesGlobals.getPirateBoldOutlineFont(), text_fg = (1, 1, 1, 1), text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, text_scale = textScale, text_pos = (self.guiWidth * 0.5, self.guiHeight * 0.94999999999999996 - textScale), modelName = 'pir_m_gui_frm_main_blue', showHeadBoard = True, nameTag = '---')
        self.cartFrame.setPos(0.0, 0.0, 0.0)
        self.frontTabParent = self.attachNewNode('frontTab', sort = 2)
        self.frontTabParent.setPos(-0.5 * self.guiWidth, 0.0, -0.40000000000000002 * self.guiHeight)
        currencyIcon = choice(self.pvpMode, self.RenownImage, self.CoinImage)
        self.myGold = DirectFrame(parent = self.cartFrame, relief = None, text = '000000', text_fg = PiratesGuiGlobals.TextFG2, text_align = TextNode.ACenter, text_scale = PiratesGuiGlobals.TextScaleLarge, text_pos = (0, 0), text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, image = currencyIcon, image_scale = 0.14999999999999999, image_pos = (0.20000000000000001, 0, 0.014), pos = (0.25 * self.guiWidth, 0, -0.34999999999999998 * self.guiHeight))
        self.sellButton = GuiButton.GuiButton(parent = self.cartFrame, text = PLocalizer.SimpleStoreSell, text_fg = PiratesGuiGlobals.TextFG2, text_pos = (0.0, -0.014), text_scale = PiratesGuiGlobals.TextScaleLarge, text_align = TextNode.ACenter, text_shadow = PiratesGuiGlobals.TextShadow, image = GuiButton.GuiButton.blueGenericButton, image_scale = (0.59999999999999998, 0.59999999999999998, 0.59999999999999998), command = self.showSellGUI)
        self.sellButton.setPos(0.28000000000000003 * self.guiWidth, 0, -0.40000000000000002 * self.guiHeight)
        charGui = loader.loadModel('models/gui/char_gui')
        self.rotateSlider = DirectSlider(parent = base.a2dBottomLeft, relief = None, command = self.rotatePreviewPirate, image = charGui.find('**/chargui_slider_small'), image_scale = (2.1499999999999999, 2.1499999999999999, 1.5), thumb_relief = None, thumb_image = (charGui.find('**/chargui_slider_node'), charGui.find('**/chargui_slider_node_down'), charGui.find('**/chargui_slider_node_over')), pos = (0.80000000000000004, 0.0, 0.089999999999999997), text_align = TextNode.ACenter, text_scale = (0.10000000000000001, 0.10000000000000001), text_pos = (0.0, 0.10000000000000001), text_fg = PiratesGuiGlobals.TextFG1, scale = 0.42999999999999999, text = PLocalizer.RotateSlider, value = 0.5, sortOrder = -1)
        ...
        main_gui.removeNode()
        self.closeButton = GuiButton.GuiButton(parent = self, relief = None, pos = (0.82999999999999996, 0, -0.14000000000000001), image = (generic_box, generic_box, generic_box_over, generic_box), image_scale = 0.40000000000000002, command = self.closePanel)
    ...
    def focusCamera(self):
        ...
        if self.previewItem:
            ...
            dummy.wrtReparentTo(render)
            dummy.lookAt(self.previewPirate, self.previewPirate.headNode.getX(self.previewPirate), self.previewPirate.headNode.getY(self.previewPirate), self.previewPirate.headNode.getZ(self.previewPirate) * 0.90000000000000002)
        dummy.setH(dummy, -17)
        dummy.setP(dummy, -12)
        camPos = dummy.getPos()
    ...
    def addTab(self, tabId, displayName = '', text = '', image = None, image_scale = None):
        ...
        if image:
            textPosition = (-0.070000000000000007, -0.20000000000000001, 0.0)
        else:
    ...
    def itemClicked(self, itemId, task = None):
        ...
        if isinstance(self.clickedItem, SimpleClothingItem) and ItemGlobals.canDyeItem(itemId):
            ...
            InventoryUIStoreContainer.InventoryUIStoreContainer.detailsHeight = 0.56000000000000005
        self.showClicked()
        if not self.clickedItem:
            self.itemCardPlaceholder.show()
...
class CatalogStoreGUI(SimpleStoreGUI):
    ...
    def createPreviewBuffer(self):
        ...
        self.previewLens.setAspectRatio(0.64000000000000001 / 0.86499999999999999)
