...
class JewelryStoreGUI(DirectFrame):
    ...
    def __init__(self, npc, shopId, **kw):
        ...
        self.purchaseInventory = JewelryStoreCartList(self, self.cartWidth, self.cartHeight - 0.94999999999999996, self.cartWidth, self.cartHeight / 20.0)
        self.purchaseInventory.reparentTo(self.cartFrame)
        self.purchaseInventory.setPos(0.0, 0.0, 0.76000000000000001)
        self.sellInventory = JewelryStoreCartList(self, self.cartWidth, self.cartHeight - 0.94999999999999996, self.cartWidth, self.cartHeight / 20.0)
        ...
        self.balanceValue = DirectFrame(parent = self.cartFrame, relief = None, text = str(self.balance), text_fg = PiratesGuiGlobals.TextFG2, text_align = TextNode.ARight, text_scale = PiratesGuiGlobals.TextScaleLarge, text_pos = (-0.055, 0.0), text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, image = self.CoinImage, image_scale = 0.14999999999999999, image_pos = (-0.025000000000000001, 0, 0.014999999999999999), pos = (self.cartWidth, 0, 0.22500000000000001))
        self.myGoldTitle = DirectFrame(parent = self.cartFrame, relief = None, text = PLocalizer.YourMoney, text_fg = PiratesGuiGlobals.TextFG2, text_align = TextNode.ALeft, text_scale = PiratesGuiGlobals.TextScaleLarge, text_pos = (0.0, 0.0), text_shadow = PiratesGuiGlobals.TextShadow, pos = (0.089999999999999997, 0, 0.155))
        self.myGold = DirectFrame(parent = self.cartFrame, relief = None, text = str(localAvatar.getMoney()), text_fg = PiratesGuiGlobals.TextFG2, text_align = TextNode.ARight, text_scale = PiratesGuiGlobals.TextScaleLarge, text_pos = (-0.055, 0.0), text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = 1, image = self.CoinImage, image_scale = 0.14999999999999999, image_pos = (-0.025000000000000001, 0, 0.014999999999999999), pos = (self.cartWidth, 0, 0.155))
    ...
    def setPage(self, pageName, startIndex = 0, refreshWardrobe = True):
        ...
        for jewel in jewelry:
            if self.jewelryAmount - startIndex < self.buttonsPerPage and self.jewelryAmount >= startIndex:
                ...
                jewelryButton.itemText = DirectFrame(parent = jewelryButton, relief = None, text = '', text_fg = PiratesGuiGlobals.TextFG1, text_align = TextNode.ACenter, text_scale = PiratesGuiGlobals.TextScaleSmall, text_shadow = PiratesGuiGlobals.TextShadow, textMayChange = True, pos = (-0.13, 0, -0.085000000000000006))
                ...
                if not self.paid:
                    ...
                    jewelryButton.addToCart['geom_pos'] = Vec3(-0.10000000000000001, 0.0, 0.0)
        ...
        for x in range(len(regionData)):
            if pageName == JewelryGlobals.RBROW:
                ...
                if gender == 'f':
                    ...
                    offsetZ = -rightEyeHeight * 0.98999999999999999
                ...
                if gender == 'f':
                    ...
                    offsetZ = -leftEarHeight * 0.98999999999999999
