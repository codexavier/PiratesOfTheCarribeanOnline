...
class NPCMoviePlayer(DirectObject.DirectObject):
    ...
    def parseLine(self, command, line, chapterList, timeoutList, lineNum = -1):
        ...
        if self.isLocalToon:
            if command == 'LOAD':
                ...
                iList.append(self.parseCreateMsgPanel(line, 0.20000000000000001, 0, 0.20000000000000001))
            elif command == 'CREATE_CHAT_TUT_PANEL':
                iList.append(self.parseCreateMsgPanel(line, 2.0, -1.0, -0.59999999999999998))
    ...
    def parseCreateMsgPanel(self, line, x = -1.2, y = -1, z = -0.59999999999999998):
        lineLen = len(line)
        scriptYes = None
        scriptNo = None
        if lineLen == 2:
