...
class DistributedCellDoor(DistributedInteractive.DistributedInteractive):
    ...
    def doorOpen(self):
        ...
        self.doorTrack = Sequence(Func(base.playSfx, self.OpenSfx, node = self.doorGeomNodePath), LerpHprInterval(self.doorGeomNodePath, 0.20000000000000001, hpr = hpr, startHpr = startHpr))
    ...
    def doorShut(self):
        ...
        self.doorTrack = Sequence(Func(base.playSfx, self.CloseSfx, node = self.doorGeomNodePath), LerpHprInterval(self.doorGeomNodePath, 0.20000000000000001, hpr = hpr, startHpr = startHpr))
        self.doorTrack.start()
    ...
    setHealth = report(types = [
        'frameCount',
        'args'], dConfigParam = 'jail')(setHealth)
