...
class InventoryUIContainer(DirectFrame, InventoryRequestBase.InventoryRequestBase):
    testItemCount = 0
    cellCount = 0
    detailsDelay = 0.29999999999999999
    detailsPos = None
    detailsHeight = None
    ...
    def setTitleInfo(self):
        ...
        self.titleImageOpen.setScale(0.17000000000000001)
        self.titleImageClosed.setScale(0.17000000000000001)
    ...
    def makeCell(self, cellImage = None, imageScale = None, imagePos = None, frameScale = (1.0, 1.0), offset = (0.0, 0.0)):
        ...
        cell['extraArgs'] = [
            cell]
        cellLabel = DirectLabel(parent = cell, textMayChange = 1, text = '', text_font = PiratesGlobals.getPirateBoldOutlineFont(), text_fg = PiratesGuiGlobals.TextFG2, text_scale = textScale, text_align = TextNode.ARight, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.45000000000000001 * self.cellSizeX - 0, -0.65000000000000002 * self.cellSizeZ + textScale))
        cellLabel.hide()
        cell.labelText = ''
    ...
    def makeCellLabel(self, cell):
        if cell.label.isEmpty():
            cellLabel = DirectLabel(parent = cell, textMayChange = 1, text = '', text_font = PiratesGlobals.getPirateBoldOutlineFont(), text_fg = PiratesGuiGlobals.TextFG2, text_scale = textScale, text_align = TextNode.ARight, text_shadow = PiratesGuiGlobals.TextShadow, text_pos = (0.45000000000000001 * self.cellSizeX - 0, 0.45000000000000001 * self.cellSizeZ - textScale))
            cell.label = cellLabel
    ...
    def resizeItem(self, item):
        sizeProp = 0.46000000000000002
        textScale = 0.25 * self.cellSizeX
        item.setScale(1, 1, 1)
        item['frameSize'] = (self.cellSizeX * -sizeProp, self.cellSizeX * sizeProp, self.cellSizeZ * -sizeProp, self.cellSizeZ * sizeProp)
        item['borderWidth'] = (self.cellSizeX * 0.050000000000000003, self.cellSizeX * 0.050000000000000003)
        item['text_scale'] = textScale * item.textScale
        item['text_pos'] = (0.0, self.cellSizeZ * item.textOffset + textScale * 0.34999999999999998)
        item['image_scale'] = 0.90000000000000002 * self.cellSizeX * item.imageScale
    ...
    def takeOut(self):
        messenger.send('openingContainer', [
            self])
